...
private:
    ...
    void InitHypergraph()
    {
        ...
        _vecNodeGain = std::vector<double>(_numV);
...
public:
    ...
    /// _FRsets4Edges
    FRsets _FRsets4Edges;
    RRsets _RRsets4Edges;

    ProbDist _probDist = WEIGHTS;

    std::vector<double> _vecUtility;
    std::vector<double> _vecNodeGain;
    std::vector<double> _vecEdgeGain;
    /// acprob_by_seed: the activation probability of each RR set by each seed
    std::vector<std::unordered_map<uint32_t, double>> _acprob_by_seed;


    explicit HyperGraph(Graph& graph) : _graph(graph)
    {
        InitHypergraph();
    }
...
finished:
    ...
    void updateUtility(std::unordered_map<uint32_t, double>& mapSeedProb) {
        size_t prev_numRR = this->_vecUtility.size();
        size_t curr_numRR = this->_numRRsets;
        for (int i = prev_numRR; i < curr_numRR; i++) {
            double utility = 1.0;
            this->_acprob_by_seed.push_back(std::unordered_map<uint32_t, double>());
            for (auto node : _RRsets[i]) {
                if (mapSeedProb.find(node) != mapSeedProb.end()) {
                    utility *= (1.0 - mapSeedProb[node]);
                    this->_acprob_by_seed[i][node] = -1.0; // mark as negative
                }
            }
            this->_vecUtility.push_back(utility);
            for (auto node : _RRsets[i]) {
                this->_vecNodeGain[node] += utility;
            }
        }
        return;
    }

    // Compute the influence upper bound of given edges
    double ComputeInfUpperBound(const std::vector<UVWEdge>& vecEdges, std::unordered_map<uint32_t, double>& mapSeedProb)
    {
        std::vector<double> vecRRsetValue(_numRRsets, 0.0);
        std::vector<bool> vecBoolVst = std::vector<bool>(_numRRsets, false);
        for (auto& edge : vecEdges)
        {
            uint32_t st_seed = edge.first, new_node = edge.second.first;
            double prob = edge.second.second;

            for (auto RRIdx : _FRsets[new_node]) {
                vecRRsetValue[RRIdx] += ((1.0 - vecRRsetValue[RRIdx]) * prob);
            }
        }
        for (auto& edge : vecEdges) {
            uint32_t new_node = edge.second.first;
            for (auto RRIdx : _FRsets[new_node]) {
                if (!vecBoolVst[RRIdx]) {
                    vecRRsetValue[RRIdx] *= this->_vecUtility[RRIdx];
                    vecBoolVst[RRIdx] = true;
                }
            }
        }
        // LogInfo("connected RR sets", std::accumulate(vecBoolVst.begin(), vecBoolVst.end(), 0));
        double cov_val = std::accumulate(vecRRsetValue.begin(), vecRRsetValue.end(), 0.0);
        LogInfo("cov val for UB valid", cov_val);
        return 1.0 * cov_val * _numV / _numRRsets;
    }

    // Compute the influence lower bound of given edges
    // double ComputeInfLowerBound(const std::vector<UVWEdge>& vecEdges, std::unordered_map<uint32_t, double>& mapSeedProb)
    // {
    //     LogInfo("# Edges", vecEdges.size());
    //     std::vector<double> vecRRsetValue(_numRRsets, 0.0);
    //     std::vector<bool> vecBoolVst = std::vector<bool>(_numRRsets, false);
    //     for (auto& edge : vecEdges)
    //     {
    ...
    //         double prob = edge.second.second;
    //         for (auto RRIdx : _FRsets[new_node]) {
    //             vecRRsetValue[RRIdx] += ((1.0 - vecRRsetValue[RRIdx]) * prob * mapSeedProb[st_seed]);
    //         }
    //     }
    //     for (auto& edge : vecEdges) {
    //         uint32_t new_node = edge.second.first;
    ...
    //     double cov_val = std::accumulate(vecRRsetValue.begin(), vecRRsetValue.end(), 0.0);
    //     LogInfo("cov val for LB valid", cov_val);
    //     return 1.0 * cov_val * _numV / _numRRsets;
    // }
    double ComputeInfLowerBound(const std::vector<UVWEdge>& vecEdges, std::unordered_map<uint32_t, double>& mapSeedProb)
    {
        ...
        std::vector<double> vecRRsetValue(_numRRsets, 0.0);
        std::vector<bool> vecBoolVst = std::vector<bool>(_numRRsets, false);
        std::vector<size_t> connectedRRsets;
        Graph nodeToEdges(this->_numV);
        double cov = 0.0;
        for (auto& edge : vecEdges)
        {
            uint32_t st_seed = edge.first, new_node = edge.second.first;
            double prob = edge.second.second;
            nodeToEdges[new_node].push_back(std::make_pair(st_seed, prob));
            for (auto RRIdx : _FRsets[new_node]) {
                if (vecBoolVst[RRIdx]) continue;
                connectedRRsets.push_back(RRIdx);
                vecBoolVst[RRIdx] = true;
            }
        }
        LogInfo("connected RR sets", connectedRRsets.size());
        // compute the probability that each RR set will be activated by a seed
        for (auto RR_idx : connectedRRsets) {
            for (auto node : _RRsets[RR_idx]) {
                for (auto& edge: nodeToEdges[node]) {
                    uint32_t st_seed = edge.first;
                    double prob = edge.second;
                    if (this->_acprob_by_seed[RR_idx][st_seed] < -0.5) continue;
                    this->_acprob_by_seed[RR_idx][st_seed] += (1-this->_acprob_by_seed[RR_idx][st_seed])*prob;
                }
            }
        }
        for (auto RR_idx : connectedRRsets) {
            double prob_non_ac = 1.0;
            for (auto& seed_prob : this->_acprob_by_seed[RR_idx]) {
                if (seed_prob.second < -0.5) continue;
                prob_non_ac *= (1 - mapSeedProb[seed_prob.first] * seed_prob.second);
            }
            double gain = (1.0 - prob_non_ac)*this->_vecUtility[RR_idx];
            cov += gain;
        }
        // for (int i=0; i<10; i++) {
        //     LogInfo("utility", this->_vecUtility[connectedRRsets[i]]);
        //     double prob_non_ac = 1.0;
        //     for (auto& seed_prob : this->_acprob_by_seed[connectedRRsets[i]]) {
        //         if (seed_prob.second < 0.0) continue;
        //         prob_non_ac *= (1 - mapSeedProb[seed_prob.first] * seed_prob.second);
        //     }
        //     LogInfo("non-ac probability", prob_non_ac);
        //     LogInfo("gain", (1.0 - prob_non_ac)*this->_vecUtility[connectedRRsets[i]]);
        // }
        // for (auto& edge : vecEdges) {
        //     uint32_t new_node = edge.second.first;
        //     for (auto RRIdx : _FRsets[new_node]) {
        ...
        LogInfo("cov val for LB valid", cov);
        return 1.0 * cov * _numV / _numRRsets;
    }
    
    double BuildOneRRsetEstimation(const uint32_t uStart, std::unordered_map<uint32_t, double>& mapSeedProb)
    {
        size_t numVisitNode = 0, currIdx = 0;
        double res = 0.0;
        _vecVisitNode[numVisitNode++] = uStart;
        ...
        for (int i = 0; i < numVisitNode; i++) {
            if (mapSeedProb.find(_vecVisitNode[i]) != mapSeedProb.end()) {
                res += (1.0 - res)*mapSeedProb[_vecVisitNode[i]];
            }
            _vecVisitBool[_vecVisitNode[i]] = false;
    ...
    double BuildRRsetsEstimation(const size_t num_samples, std::unordered_map<uint32_t, double>& mapSeedProb) {
        double res = 0.0;
        for (auto i = 0; i < num_samples; i++) {
            res += BuildOneRRsetEstimation(dsfmt_gv_genrand_uint32_range(_numV), mapSeedProb);
        }
        return res * _numV / num_samples;
    }

    double EvalInfProbSeeds(std::unordered_map<uint32_t, double>& mapSeedProb) {
        std::vector<double> vecRRsetValue(_numRRsets, 0.0);
        double res = 0.0;
        for (auto& seed_prob : mapSeedProb) {
            uint32_t seed = seed_prob.first;
            double prob = seed_prob.second;
            for (auto RRIdx : _FRsets[seed]) {
                double marginal_prob = (1.0 - vecRRsetValue[RRIdx]) * prob;
                vecRRsetValue[RRIdx] += marginal_prob;
                res += marginal_prob;
            }
        }
        return 1.0 * res * _numV / _numRRsets;
    }

    void updateCoverage4Edges(const Graph& candEdges, const std::vector<size_t>& st_idx_per_node, size_t numCandEdges) {
        if (this->_FRsets4Edges.size() == 0) {
            this->_FRsets4Edges = FRsets(numCandEdges);
            this->_vecEdgeGain = std::vector<double>(numCandEdges, 0.0);
        }
        assert(this->_acprob_by_seed.size() == this->_numRRsets);
        size_t prev_numRR = this->_RRsets4Edges.size();
        size_t curr_numRR = this->_numRRsets;

        for (size_t i = prev_numRR; i < curr_numRR; i++) {
            std::vector<uint32_t> vec_edge_idx;
            for (auto node : _RRsets[i]) {
                for (uint32_t j = 0; j < candEdges[node].size(); j++) {
                    auto& edge = candEdges[node][j];
                    size_t edge_idx = st_idx_per_node[node] + j;
                    uint32_t u = edge.first;
                    // see if the seed is in the RRset
                    if (this->_acprob_by_seed[i][u] < -0.5) {
                        continue;
                    }
                    this->_FRsets4Edges[edge_idx].push_back(i);
                    vec_edge_idx.push_back(edge_idx);
                    this->_vecEdgeGain[edge_idx] += _vecUtility[i];
                }
            }
            this->_RRsets4Edges.push_back(RRset(vec_edge_idx.begin(), vec_edge_idx.end()));
            // LogInfo("RR set for edges size", vec_edge_idx.size());
            // LogInfo("RR set size", this->_RRsets[i].size());
            // LogInfo("RR utility", _vecUtility[i]);
        }
    }
};
